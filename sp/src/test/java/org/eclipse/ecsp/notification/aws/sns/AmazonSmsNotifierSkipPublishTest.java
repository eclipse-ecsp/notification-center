 ...
 * AWS SNS test.
 */
public class AmazonSmsNotifierSkipPublishTest {
    ...
    public void doPublish_success() throws Exception {
        ...
        AmazonSmsNotifierSkipPublish amazonSmsNotifierSkipPublish = new AmazonSmsNotifierSkipPublish();

        ChannelResponse channelResponse = amazonSmsNotifierSkipPublish.doPublish(alert);
        ...
        assertEquals(channelResponse.getAlertData().getDefaultMessage(),
            alert.getNotificationTemplate().getChannelTemplates().getSms().getBody());
    }
}